'use strict'
/**
 * Integrations (backend port) — HTTP request, API personalizada y acción CRM.
 * Antes estos nodos eran stubs en el servidor, así que el módulo HTTP no
 * ejecutaba ni guardaba la respuesta. Ahora corren de verdad.
 */
...
const integrationNodes = [
  ...
  { type: 'sql', category: 'integrations', label: 'SQL', async exec() { throw new Error('SQL aún no implementado — usa un nodo HTTP Request hacia tu backend.') } },
  { type: 'email_send', category: 'integrations', label: 'Email', async exec() { throw new Error('Email aún no implementado — usa un nodo HTTP Request hacia un servicio de email.') } },
  { type: 'erp', category: 'integrations', label: 'ERP', async exec() { throw new Error('ERP aún no implementado — usa un nodo HTTP Request directo.') } },
]
