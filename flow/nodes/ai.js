...
function buildPmsToolDefs(account) {
  ...
  defs.push(
    { type: 'function', function: { name: 'ver_habitaciones',
      description: `Lista las habitaciones${hotel}. Es tu ÚNICA fuente sobre qué alojamientos existen y qué tienen: NUNCA menciones ni describas uno que no haya salido de aquí, y NUNCA afirmes que una habitación tiene algo si esta herramienta no lo ha dicho.\n· Sin parámetros → la lista completa, en texto.\n· "busco"=<lo que pide el cliente con sus palabras> → responde EN TEXTO cuáles lo tienen de verdad. Úsalo SIEMPRE que pregunte si hay algo ("¿tienen jacuzzi?", "algo para 4", "con vista al mar"). Si responde que NINGUNA lo tiene, díselo tal cual: no ofrezcas otra como sustituta ni digas que "podría" tenerlo.\n· "habitacion"=<nombre EXACTO de la lista> → envía las FOTOS y la ficha de esa. SOLO si el cliente pidió ver fotos.\n· "fotos"=true → panorama de fotos. SOLO si pidió fotos.\nPreguntar NO es pedir fotos: si solo quiere saber si tienen algo, usa "busco" y contéstale con palabras.`,
      parameters: { type: 'object', properties: {
        busco: { type: 'string', description: 'Lo que el cliente busca con sus palabras ("alguna con jacuzzi", "algo para 4 personas", "con vista al mar"). Responde EN TEXTO qué habitaciones lo tienen. NO envía fotos. Si ninguna lo tiene, lo dice — y entonces hay que decírselo al cliente, no ofrecer otra.' },
        habitacion: { type: 'string', description: 'Nombre EXACTO de una habitación (de las que devolvió esta herramienta) para ENVIAR SUS FOTOS y ficha. SOLO si el cliente pidió ver fotos. Para buscar por características usa "busco", no esto.' },
        fotos: { type: 'boolean', description: 'true SOLO si el cliente pide ver fotos (envía el panorama). Por defecto la función solo lista en texto.' },
