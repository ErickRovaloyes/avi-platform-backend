...
function buildPmsToolDefs(account) {
  ...
  defs.push(
    { type: 'function', function: { name: 'ver_habitaciones',
      description: `Lista las habitaciones/tipos${hotel} (nombre, capacidad y descripción) EN TEXTO. Es tu ÚNICA fuente sobre qué alojamientos existen: tú no los conoces, NUNCA menciones uno que no haya salido de aquí. Tres usos: (1) sin parámetros, para "¿qué habitaciones tienen?"; (2) con "busco"=<lo que pide el cliente con sus palabras> cuando describa lo que necesita sin nombrar ninguna ("alguna con jacuzzi", "algo para 4", "con vista al mar") — responde EN TEXTO con las que encajan; (3) con "habitacion"=<nombre> SOLO cuando el cliente pida VER FOTOS de una concreta. Si solo pregunta si tienen algo, usa "busco" y contéstale con palabras: no mandes fotos si no las ha pedido. NO envía fotos por defecto. Envía FOTOS solo si el cliente las pide: "habitacion"=<nombre> para las fotos+ficha de una, o "fotos"=true para el panorama. Cada envío de fotos manda fotos NUEVAS; si el cliente pide "más fotos", vuelve a llamarlo; con desde_inicio=true reenvía desde el principio.`,
      parameters: { type: 'object', properties: {
        busco: { type: 'string', description: 'Lo que el cliente busca, con sus palabras ("alguna con jacuzzi", "algo para 4 personas", "con vista al mar"). Devuelve EN TEXTO las habitaciones que encajan. NO envía fotos.' },
        habitacion: { type: 'string', description: 'Nombre de una habitación concreta para ENVIAR SUS FOTOS y ficha. Úsalo SOLO si el cliente pidió ver fotos (vacío = solo lista/panorama)' },
