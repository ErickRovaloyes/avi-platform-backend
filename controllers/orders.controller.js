...
const saveConfig = async (req, res) => {
  ...
  try {
    ...
    const b = req.body || {}
    const next = { ...cur }
    for (const k of ['enabled', 'orderTypes', 'currency', 'taxPct', 'packagingFee', 'minOrder', 'freeDeliveryThreshold', 'paymentMethods', 'notifyTeam', 'postOrderFlowId', 'tips', 'businessName', 'notifyCustomer', 'statusMessages', 'hours', 'reviewEnabled', 'reviewUrl', 'reviewMessage']) {
...
const saveProduct = async (req, res) => {
  ...
  try {
    ...
    const comboItems = (Array.isArray(b.comboItems) ? b.comboItems : []).filter(c => c && c.name).map(c => ({ productId: c.productId || '', name: String(c.name).slice(0, 160), qty: Math.max(1, Number(c.qty) || 1) })).slice(0, 30)
    const vals = { category: String(b.category || '').slice(0, 120), name: String(b.name || '').slice(0, 200), description: String(b.description || ''), price: Number(b.price) || 0, promo_price: Math.max(0, Number(b.promoPrice) || 0), combo_items: JSON.stringify(comboItems), media_id: b.mediaId || null, image_url: String(b.imageUrl || ''), modifier_group_ids: JSON.stringify(Array.isArray(b.modifierGroupIds) ? b.modifierGroupIds : []), available: b.available === false ? 0 : 1, sort: Number(b.sort) || 0 }
    if (b.id) {
      ...
      await pool.query('INSERT INTO order_products (id,account_id,category,name,description,price,promo_price,combo_items,media_id,image_url,modifier_group_ids,available,sort,source,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [id, accId, vals.category, vals.name, vals.description, vals.price, vals.promo_price, vals.combo_items, vals.media_id, vals.image_url, vals.modifier_group_ids, vals.available, vals.sort, 'menu', Date.now()])
    }
    socket.emit(accId, 'account:updated', { accId })
    res.json({ ok: true, id })
