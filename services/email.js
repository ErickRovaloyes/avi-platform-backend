...
async function loadEmailConfig() {
  try {
    const [[r]] = await pool.query('SELECT email_provider, email_api_key, email_from, email_from_name FROM platform_settings WHERE id=1')
    if (!r) return null
    return {
...
function isConfigured(cfg) {
  return !!(cfg && cfg.provider && cfg.provider !== 'none' && cfg.apiKey && cfg.from)
}
